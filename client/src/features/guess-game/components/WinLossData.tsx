...
export function WinLossData({ isWin }: Props) {
  ...
  return (
    <div className='flex flex-col gap-6 text-center'>
      ...
      <div>
        <p className='text-center text-green-700 font-bold text-xl md:text-xl'>Wins: {wins ?? 'N/A'}</p>
        <p className='text-center text-red-800 font-bold text-xl md:text-xl'>Losses: {losses ?? 'N/A'}</p>
      </div>
    </div>
  );
}
