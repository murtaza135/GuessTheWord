...
function authenticate({ strategy, session = true, message }: AuthOptions) {
...
function protect({ message }: Pick<AuthOptions, 'message'>) {
  return authenticate({ strategy: 'protect', session: true, message });
}

function connect({ strategy, message }: Omit<AuthOptions, 'session'>) {
  return authenticate({ strategy, session: false, message });
}

function startOAuth({ strategy, scope }: StartOAuthOptions) {
